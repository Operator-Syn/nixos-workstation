...
let
  ...
  aclEntries =
    policy:
    if policy.readerGroup == null then [ "u:${policy.reader}" ] else [ "g:${policy.readerGroup}" ];
  directoryAcl =
    policy:
    concatStringsSep "," (
      map (entry: "${entry}:r-x") (aclEntries policy) ++ map (entry: "d:${entry}:r-X") (aclEntries policy)
    );
  fileAcl = policy: concatStringsSep "," (map (entry: "${entry}:r-X") (aclEntries policy));
  ...
  mkApplyScript =
    ...
    pkgs.writeShellScript "home-acl-apply-${policy.name}-${mode}" ''
      ...
      acl_mode='${if mode == "directories" then directoryAcl policy else fileAcl policy}'
  ...
  mkWorker =
    ...
    pkgs.writeShellScript "home-acl-repair-subtree-${policy.name}" ''
      ...
      "$find" "$subtree" -xdev \
        \( -type d \( ${cacheNameExpression} \) -prune \) -o \
  ...
  mkRepairScript =
    ...
    pkgs.writeShellScript "home-acl-repair-${policy.name}" ''
      ...
      "$setfacl" -m '${directoryAcl policy}' "$home"

      # Apply direct children serially, then repair independent directory trees
      # with a bounded worker pool. This avoids overlapping ACL mutations.
  ...
  repairCommand = pkgs.writeShellScriptBin "hermes-repair-acls" ''
    ...
    run_policy() {
      ...
      "$sudo" "$journalctl" --no-pager -n 0 -f -u "$unit" &
      journal_pid=$!
